...
public class ProductDaoImpl implements ProductDao {
	...
	public void saveReport(DataSource dataSource, int supplierId, int userId,
			String company, String reportName, ArrayList<Product> products, ArrayList<Inventory_Transaction> it) {
		try {
			...
			if(it.size() > 0){
				if(it !=null && it.get(0) !=null){
					...
					for(Inventory_Transaction i: it){
						...
						i.getTransaction_created_date() + "," + i.getTransaction_modified_date() +
						"," + i.getProduct_id() + "," + i.getQuantity() + "," + i.getPurchase_order_id() +
						"," + i.getCustomer_order_id() + "," + i.getComments() + "\n");
					}
				}
			}
	...
	public ArrayList<Inventory_Transaction> getInventoryTransactions(
		...
		System.out.println("newDate : " + newDate + "sqlDate : " + new java.sql.Date(newDate.getTime()));
		try {
			connection = dataSource.getConnection();
			PreparedStatement query = connection
			...
			for(Inventory_Transaction i: it_List)
				System.out.println(i.getId() + " : " + i.getProduct_id() + " : " + i.getTransaction_created_date());
			connection.close();
